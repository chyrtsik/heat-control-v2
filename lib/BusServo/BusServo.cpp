...
const uint32_t c_CycleCompensation = 30;  // compensation us to trim adjust for digitalWrite delays
...
static void Servo_Handler(T* timer)
{
    ...
    if (timer->isEndOfCycle()) {
        ...
        if (servoIndex < s_servoCount && s_servos[servoIndex].info.isActive) {
            ...
            bus_digitalWrite(s_servos[servoIndex].info.pin, LOW);
    ...
    if (servoIndex < s_servoCount &&
        ...
        if (s_servos[servoIndex].info.isActive) {
            if (s_servos[servoIndex].info.isDetaching) {
            	...
            	bus_digitalWrite(s_servos[servoIndex].info.pin, HIGH);
            }
        }
    }
    else {
...
uint8_t Servo::attach(int pin, int minUs, int maxUs)
{
    ...
    if (_servoIndex < MAX_SERVOS) {
        ...
        // reasonable ranges
        _maxUs = 2400;//TODO: _max(250, _min(3000, maxUs));
        _minUs = 544;//TODO: _max(200, _min(_maxUs, minUs));
